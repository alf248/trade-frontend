...
function OfferList(props) {
    ...
    const {offers, error, isLoaded} = props
...
function TableRow(props)  {
    ...
    const {id, name, price, currency, image, creatorFID, receiverFID, sale, creatorName} = offer
    ...
    const cName = (creatorName && creatorName !== "") ? creatorName : null

    let fromLink = null
    //if (offer.receiver !== "") from = offer.receiver
    if (sale) {
        fromLink = <Link to={`/users/${creatorFID}`} style={{fontSize: "80%"}}>{cName ? "by "+cName : "view seller"}</Link>
    } else {
        fromLink = <Link to={`/users/${creatorFID}`} style={{fontSize: "80%"}}>{cName ? "by "+cName : "view buyer"}</Link>
    }
    ...
    return (
        ...
        <Row>
            ...
            <Col xs={0} sm={0} md={3} className="color-plate-grey">
